...
class MessageSplitterPlugin(Star):
    ...
    def _remember_incoming_message(self, event: AstrMessageEvent) -> None:
        ...
        if len(queue) > self._QUEUE_MAX_SIZE: queue.popleft()
    ...
    async def on_message(self, event: AstrMessageEvent):
        """记录 incoming 消息 ID，用于智能回复判定（仅在用户消息被他人插话时附加 Reply）。"""
    ...
    async def on_decorating_result(self, event: AstrMessageEvent):
        ...
        if not result or not result.chain:
            self._log_debug(event, "跳过: result 或 chain 为空")
            return
        if getattr(result, "__splitter_processed", False):
            self._log_debug(event, "跳过: 已处理过")
            return
        ...
        if max_len_disable > 0 and total_text_len > max_len_disable:
            self._log_debug(event, f"跳过: 文本过长 ({total_text_len}>{max_len_disable})")
            return
            ...
            if not has_non_plain:
                ...
                if pre_split_text.strip():
                    try:
                        ...
                        if llm_split_provider:
                            provider_id = llm_split_provider.strip()
                        else:
                            provider_id = await self.context.get_current_chat_provider_id(umo=event.unified_msg_origin)
                        if provider_id:
                            prompt = f"{llm_split_prompt}\n\n{pre_split_text}"
                            resp = await self.context.llm_generate(
                                chat_provider_id=provider_id,
                                prompt=prompt,
                            )
                            processed_text = resp.completion_text
                            if processed_text and processed_text.strip():
                                clean_after_regex = self._get_cfg("clean_after_regex", "")
                        ...
                        logger.error(f"{self._tag(event)} LLM分段失败: {e}")
        ...
        self._log_debug(event, f"切分完成: {len(segments)}段, text_len={total_text_len}")
        ...
        for i in range(len(segments) - 1):
            ...
            try:
                ...
                logger.error(f"{self._tag(event)} 发送失败: {e}")
    ...
    def _log_segment(self, event: AstrMessageEvent, index: int, total: int, chain: List[BaseMessageComponent], method: str):
        content = "".join([c.text if isinstance(c, Plain) else f"[{type(c).__name__}]" for c in chain])
        logger.info(f"{self._tag(event)} 第 {index}/{total} 段 ({method}): {content.replace(chr(10), chr(92) + 'n')}")
